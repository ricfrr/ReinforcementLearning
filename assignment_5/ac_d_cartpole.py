...
from ac_d_agent import Agent, Policy, Value
...
def train(env_name, print_things=True, train_run_id=0, train_episodes=5000):
    ...
    for episode_number in range(train_episodes):
        ...
        while not done:
            ...
            #agent.store_outcome(previous_observation, action_probabilities, action, reward, done)

            # Store total episode reward
            reward_sum += reward
            timesteps += 1
            if timesteps %10 == 0 or done:
                
                x = torch.from_numpy(observation).float().to(agent.train_device)

                td_target = reward + agent.gamma * agent.value(x)
                x_prec = torch.from_numpy(previous_observation).float().to(agent.train_device)
                
                critic_l = (td_target - agent.value(x_prec) )
                critic_l.backward(retain_graph=True)
                n_dist= agent.policy(x)
                action = n_dist.sample((1,))[0]
                act_prb = n_dist.log_prob(action)
                
                act_l = act_prb * critic_l
                act_l.backward(retain_graph=True)
        ...
        # call the episode finished evey ten timestep
