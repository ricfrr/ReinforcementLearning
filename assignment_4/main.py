...
# function for plotting the actions TASK 3
def plot_2d(agent):    
    discr = 100
    x_min, x_max = -2.4, 2.4
    ...
    for i in range(discr):
        for j in range(discr):
            ...
            st = np.array(st)
            p_2d[i,j] = agent.get_action(st) #get the action passing the state
...
#plot_2d(agent) # function for the 2d policy plot
plot_rewards(cumulative_rewards)
plt.ioff()
plt.tight_layout()
plt.draw()
plt.savefig("plots/train_reward_lunar_lander_3.jpg")
print('Complete')
#plt.show()
